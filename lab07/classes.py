    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if len(player.hand) > 0:
            ...
            for i in range(len(player.hand) - 1):
                if player.hand[i + 1].attack > best_card.attack and player.hand[i + 1].cardtype == 'Staff':
                    best_card = player.hand[i + 1]
...
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        self.attack -= 1000
        self.defense -= 1000
        survive = self.attack >= 0 and self.defense >= 0
        if survive:
            player.hand += [self]
